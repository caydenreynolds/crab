...
impl CrabType {
    ...
    pub(super) fn resolve(self, caller_id: &StructId, tmpls: &[CrabType]) -> compile::Result<Self> {
        let resolution_map = caller_id.tmpls.iter().zip(tmpls.iter()).fold(
            HashMap::new(),
            |resolution_map, (caller_tmpl, real_tmpl)| {
                resolution_map.finsert(caller_tmpl.clone(), real_tmpl.clone())
            },
        );
        match self.clone() {
            CrabType::SIMPLE(name) => match resolution_map.get(&StructId::from_name(name)) {
                None => Ok(self),
                Some(ct) => Ok(ct.clone()),
            },
                ...
                let resolved_tmpls =
                    inner_tmpls
                        ...
                        .try_fold(vec![], |resolved_tmpls, inner_tmpl| {
                            compile::Result::Ok(
                                resolved_tmpls.fpush(inner_tmpl.resolve(caller_id, tmpls)?),
                            )
                        })?;
...
impl From<StructId> for CrabType {
    fn from(si: StructId) -> Self {
        if si.tmpls.is_empty() {
            ...
            CrabType::TMPL(
                si.name,
                si.tmpls.into_iter().map(|tmpl| tmpl.into()).collect(),
            )
        }
    }
}
...
impl StructId {
    ...
    pub fn resolve(self, types: &[CrabType]) -> compile::Result<Self> {
        if types.len() != self.tmpls.len() {
            return Err(CompileError::WrongTemplateTypeCount(
                self.tmpls.len(),
                types.len(),
            ));
        }
    ...
    fn add_tmpls(self, types: &[CrabType]) -> compile::Result<Self> {
        Ok(Self {
            tmpls: types.iter().try_fold(vec![], |tmpls, ct| match ct {
...
impl Display for StructId {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        self.tmpls
            .iter()
            .try_for_each(|tmpl| write!(f, "_{}", tmpl))
    }
}
